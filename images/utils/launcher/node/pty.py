from dockerpty.pty import exec_create, Operation, PseudoTerminal
import sys
import os
import fcntl
import errno
...
def exec_command(
        client, container, command, interactive=True, stdout=None, stderr=None, stdin=None) -> str:
    """
    ...
    return operation.output.data.decode()
...
class ExecOperation(Operation):
    ...
    def start(self, sockets=None, **kwargs):
        ...
        if self.interactive:
            pumps.append(Pump(Stream(self.stdin), stream, wait_for_output=False))

        self.output = Stream(self.stdout)

        pumps.append(Pump(stream, self.output, propagate_close=False))
