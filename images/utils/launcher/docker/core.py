...
class DockerUtility:
    ...
    def pull_image(self, name: str, update=None):
        canonical_name = self.parse_image_name(name)
        assert canonical_name.tag

        digest = None

        for payload in self.docker_client.api.pull(canonical_name.repo, tag=canonical_name.tag, stream=True, decode=True):
